<x-admin.main.main>
    ...
    <div class="bg-light rounded p-4 p-sm-5 my-4 mx-3">
        <div class="d-flex align-items-center justify-content-between mb-3">
            <h3><a href="{{ url('admin/staff') }}"><button type="button" class="btn btn-outline-primary m-2"><i
        ...
        <form method="POST" action="{{ route('editStaff') }}" enctype="multipart/form-data">
            ...
            <div class="form-floating mb-3">
                <input type="txt" class="form-control" id="password" name="password" value="{{$staff->Password}}">
            ...
            <div class="form-floating mb-4">
                            <select class="form-select" id="floatingSelect" aria-label="Floating label select example" id="status"name="status">
                                <option value="{{$staff->Status}}"@if ($staff->Status === 1) selected @endif>Open</option>
                                <option value="{{$staff->Status}}"@if ($staff->Status === 0) selected @endif>Lock</option>
                            </select>
